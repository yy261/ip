...
When creating or proposing a commit message for the reviewed changes, follow the Git guidance in this project's `AGENTS.md` (lightweight tags unless an annotated tag is requested; commit messages should explain the rationale for the change; never commit or push without being explicitly asked).
...
4. Run the bundled generator from the repository root:
   ...
   Replace `HEAD`, `WORKTREE`, and the output path with the requested values. The comparison points can be any Git commit-ish such as `HEAD~1`, a tag, a branch, or a commit SHA. Use `WORKTREE` for the current files. If `python3` is not on PATH (e.g. on Windows), fall back to `python`.
...
`scripts/generate-split-view-diff.py` is the bundled standard-library-only generator (no pip packages required). Keep the page self-contained except for optional syntax-highlighting resources loaded by the page.

## Source

Adapted from [se-edu/skill-present-changes-visually](https://github.com/se-edu/skill-present-changes-visually) for use as a Claude Code project skill (`.claude/skills/` instead of `.codex/skills/`).
